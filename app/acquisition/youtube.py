"""Read-only YouTube historical comment acquisition."""
...
def _items(payload: Mapping[str, Any], *, field: str) -> list[object]:
    value = payload.get("items")
    if not isinstance(value, list):
        raise AcquisitionProtocolError(f"{field} items must be a list")
    return value
    ...
    async def collect_channel(
        ...
        *,
        ...
        for _ in range(max_pages):
            ...
            for raw_thread in _items(payload, field="commentThreads"):
    ...
    async def _consume_thread(
        ...
        *,
        ...
        if replies_obj is not None:
            inline_replies = _items(
                _object(replies_obj, field="thread.replies"),
                field="thread.replies",
            )
    ...
    async def _collect_all_replies(
        ...
        *,
        ...
        for _ in range(max_pages):
            ...
            for raw_reply in _items(payload, field="comments"):
                self._consume_reply(
                    _object(raw_reply, field="reply"),
                    video_id=video_id,
                    thread_id=thread_id,
